...
/* <head>                                                              */
/* ------------------------------------------------------------------ */
function head({ file, title, description, image, type = "website", scripts = [], schema = [], robots = "" }) {
...
function header(active) {
  const link = ([href, label]) => `<a href="${href}"${href === active ? ' aria-current="page"' : ""}>${label}</a>`;
  ...
  <nav class="drawer" id="drawer" aria-label="Menú móvil">
    ${[["catalogo.html", "Tienda"], ...izquierda, ...derecha, ["historia.html", "Nuestra historia"], ["blog.html", "Blog"]].map(link).join("\n    ")}
    <a class="btn" href="cita.html">Pedir cita</a>
  </nav>
`;
}
...
function pageHero({ crumb, parents, eyebrow, title, lead, actions = "", image, wide = false, caption = "", position = "" }) {
  const media = image ? `
        <figure class="page-hero__media${wide ? " page-hero__media--wide" : ""}">
          <img src="assets/img/${image.src}"${responsive(image.src, image.width, wide ? "(max-width: 900px) 100vw, 45vw" : "(max-width: 900px) 100vw, 40vw")} alt="${esc(image.alt)}" width="${image.width}" height="${image.height}" fetchpriority="high"${position ? ` style="object-position:${position}"` : ""}>
    ...
    <section class="page-hero${image ? "" : " page-hero--plain"}">
      <div class="wrap page-hero__grid">
        <div class="page-hero__text">
          ...
          <h1 class="display page-hero__title hero__in" style="--d:.16s">${title}</h1>
          <p class="lead hero__in" style="--d:.28s">${lead}</p>
          ${actions ? `<div class="hero__actions hero__in" style="--d:.4s">${actions}</div>` : ""}
        </div>${media}
      </div>
    </section>
`;
...
function page(opts) {
  return head(opts) + header(opts.active) + '\n  <main id="contenido" tabindex="-1">\n' + opts.body + "  </main>\n" + footer() + "\n</body>\n</html>\n";
}
